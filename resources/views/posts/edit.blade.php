    ...
    <div class="max-w-[500px] mx-auto py-10 px-4">
        ...
        <div class="card overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                 ...
                 <h1 class="text-sm font-bold absolute left-1/2 -translate-x-1/2">Modifier la publication</h1>
            </div>
            ...
            <form action="{{ route('posts.update', $post) }}" method="POST">
                ...
                <div class="flex p-4 gap-4 items-start bg-slate-50/30">
                    ...
                    <textarea name="contenu" rows="6" 
                              ...
                              placeholder="Quoi de neuf ?">{{ old('contenu', $post->contenu) }}</textarea>
                </div>
        ...
        @error('contenu')
            <div class="mt-4 p-3 bg-rose-50 border border-rose-100 rounded-lg text-rose-500 text-[10px] font-bold uppercase tracking-widest text-center">
                {{ $message }}
            </div>
        @enderror
    </div>
